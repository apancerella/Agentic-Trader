...
**Autonomy model: autonomous within guardrails.** Routines research, analyze, size, and execute trades directly once they clear every risk guardrail below — no separate confirmation step before an order goes in. Every trade is journaled in full and reported to the user afterward, so oversight happens through the record rather than a pre-trade approval. The one exception is `/sell-all`, which keeps an explicit confirmation step by design since it liquidates every position at once.
...
**Risk guardrails** (every trade must respect these — they're the approval, since there's no separate confirmation step):
...
- No more than **7** new trades per week

**Process flow.**
```
Scan / Watch → Analyze (technicals + fundamentals + earnings) → Size & check guardrails
   (entry, size, stop) → Execute → Journal → Report to user
```
